...
val client: OkHttpClient = OkHttpClient.Builder().connectTimeout(3, TimeUnit.SECONDS).callTimeout(10, TimeUnit.SECONDS).build()
...
fun getRuntimeInfo(runtimeUrl: String, cb: (info: RuntimeInfoResponse) -> Unit) {
    client.newCall(Request.Builder()
            .post(RequestBody.create("application/json; charset=utf-8".toMediaTypeOrNull(), "{\"action\":\"info\"}"))
            ...
            .enqueue(object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    try {
                        cb(RuntimeInfoResponse(ObjectMapper().readValue(body, RuntimeInfo::class.java), ResponseStatus.SUCCEEDED))
                    } catch (e: Exception) {
                        cb(RuntimeInfoResponse(null, ResponseStatus.FAILED))
                    }
                }
