...
import com.mendix.mendixnative.databinding.AppMenuLayoutBinding
...
class DevAppMenu(val activity: Activity, isDevModeEnabled: Boolean = false, handleReload: () -> Unit, onCloseProjectSelected: (() -> Unit)? = null) : AppMenu {
    ...
    init {
        ...
        binding.advancedSettingsButton.setOnClickListener {
            binding.advancedSettingsContainer.visibility = when (binding.advancedSettingsContainer.visibility) {
        ...
        binding.remoteDebuggingButton.text = activity.resources.getText(remoteDebugginButtonTextResource(preferences.isRemoteJSDebugEnabled))
        ...
        binding.advancedClearData.setOnClickListener {
            activity.runOnUiThread {
                clearDataWithReactContext(activity.application, (activity.application as MendixApplication).reactNativeHost) { success: Boolean ->
                    if (success) {
                        activity.runOnUiThread {
                            handleReload()
                        }
                    } else {
                        Toast.makeText(activity, "Clearing data failed.", Toast.LENGTH_LONG)
                    }
                }
            }
            dialog.dismiss()
        ...
        binding.elementInspectorButton.setOnClickListener {
            ...
            toggleElementInspector((activity.application as MendixApplication).reactNativeHost.reactInstanceManager.currentReactContext)
            dialog.dismiss()
        }
    ...
    private fun remoteDebugginButtonTextResource(isRemoteJsDebugEnabled: Boolean): Int = if (isRemoteJsDebugEnabled) {
