...
private fun getAESKey(): Key? {
    ...
    if (!keyStore.containsAlias(STORE_AES_KEY)) {
        ...
        keyGenerator.init(KeyGenParameterSpec.Builder(STORE_AES_KEY,
 ...
 * >= API.M and higher AES encryption is used with Base64 encoding to preserve bytes
 * < API.M values are Base64 encoded
 *
...
fun encryptValue(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        ...
        return Triple(Base64.encode(encryptedValue, DEFAULT),
            Base64.encode(cipher.iv, DEFAULT),
            true)
    }
    return Triple(Base64.encode(value.encodeToByteArray(), DEFAULT), null, false)
}

/**
 * Decrypts a base64 encoded and possibly AES encrypted value using the provided initialization value
 * Encryption is only available for >= API M.
 *
...
fun decryptValue(
    ...
    return Base64.decode(value, DEFAULT).decodeToString()
}
